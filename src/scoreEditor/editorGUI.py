...
fileSel.setTitle("ScoreEditor - New session")
...
# Create window
pygame.display.set_caption(f"scoreEditor - v0.1 [ALPHA] (February 2025) - <{os.path.basename(songFile)}>")
...
while running :
  for event in pygame.event.get() :
    ...
    if (event.type == pygame.KEYUP) :
      ...
      # ----------------
      if keys[pygame.K_i] :
        print("[INFO] Insertion will be available in a future release.")
    ...
    # -------------------------------------------------------------------------
      ...
      if (event.button == MOUSE_LEFT_CLICK) :
        ...
        # someWidget.leftClickDown(coord)
      ...
      if (event.button == MOUSE_LEFT_CLICK) :
        coord = pygame.mouse.get_pos()
        # someWidget.leftClickUp(coord)

    elif (event.type == pygame.MOUSEMOTION) :
      coord = pygame.mouse.get_pos()
      # someWidget.mouseMove(coord)
